  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        {/* Supporting Icon */}
        <div className="p-4 rounded-2xl bg-[var(--magic-blue)] text-white shadow-lg animate-bounce text-4xl sm:text-5xl mt-2">
          {icon}
        </div>

        {/* Word Title & Category */}
        <div className="flex flex-col items-center gap-1">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[var(--warm-gold)]/20 text-[var(--muted-ink)] dark:text-blue-200 text-[11px] font-extrabold uppercase tracking-wider">
            <Languages className="w-3.5 h-3.5 text-[var(--magic-blue)] dark:text-blue-200" />
            <span>{category}</span>
          </div>

          <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-[var(--ink)] dark:text-slate-100 mt-1">
            {title}
          </h3>

          {pronunciation && (
            <span className="text-xs font-semibold text-[var(--muted-ink)] dark:text-slate-300 italic">
        ...
        <div className="reader-soft-panel w-full p-4 rounded-2xl text-left flex flex-col gap-2">
          <div className="flex items-center gap-1.5 text-xs font-extrabold text-[var(--muted-ink)] dark:text-blue-200">
            <BookOpen className="w-4 h-4 text-[var(--story-green)]" />
          ...
          {example && (
            <div className="mt-2 pt-2 border-t reader-divider text-[11px] text-[var(--muted-ink)] dark:text-blue-200 italic">
              💡 {example}
            </div>
          )}
        </div>
        ...
        <div className="w-full flex items-center gap-2 pt-2">
          <button
            ...
            className="flex-1 py-3 px-4 rounded-[0.9rem] bg-[var(--magic-blue)] hover:bg-[#3f5f8e] text-white font-extrabold text-xs shadow-lg transition-transform hover:scale-[1.02] flex items-center justify-center gap-2"
          >
